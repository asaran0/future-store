...
function useForm(initial) {
  ...
  const validate = (fields) => {
    const errs = {};
    fields.forEach((f) => { const err = validators[f]?.(values[f] ?? ""); if (err) errs[f] = err; });
    setErrors(errs);
...
function Field({ label, name, type = "text", placeholder, form, autoComplete }) {
  return (
    <div className="fs-field">
      ...
      />
      {form.errors[name] && <span className="fs-input-error">⚠ {form.errors[name]}</span>}
    </div>
  );
}
  ...
  return (
    <>
      <div className="fs-modal-body">
        {apiError && (
          <div className="fs-form-error"><span>✕</span> {apiError}</div>
        )}
        <Field label="Email Address" name="email"    type="email"    placeholder="you@example.com" form={form} autoComplete="email" />
        <Field label="Password"      name="password" type="password" placeholder="••••••••"         form={form} autoComplete="current-password" />
        <button className="fs-btn-primary" onClick={handleSubmit} disabled={loading}>
    ...
    <>
      <div className="fs-modal-body">
        ...
        <div className="fs-field">
          ...
          <select className="fs-select" value={form.values.userType} onChange={(e) => form.set("userType", e.target.value)}>
...
export default function AuthModal() {
  ...
  useEffect(() => { if (authModal) setTab(authModal); }, [authModal]);

  if (!authModal) return null;
  const close = () => dispatch({ type: "CLOSE_AUTH_MODAL" });

  return (
    <div className="fs-modal-overlay" onClick={close}>
      <div className="fs-modal" onClick={(e) => e.stopPropagation()}>
        <div className="fs-modal-header">
          <div>
            <div className="fs-modal-icon">{tab === "login" ? "⚡" : "🚀"}</div>
            <div className="fs-modal-title">{tab === "login" ? "Welcome Back" : "Join FoxFury"}</div>
            <div className="fs-modal-sub">
              {tab === "login" ? "Sign in to your FoxFury account" : "Create your account to get started"}
            </div>
          </div>
